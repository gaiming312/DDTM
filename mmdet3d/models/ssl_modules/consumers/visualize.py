...
def load_numbers_from_txt(file_path):
...
class Vis3D():
    ...
    def forward(self, ssl_obj, batch_dict):
        ...
        with torch.no_grad():
            ...
            iter = batch_dict['iter']
            image_path = img_metas[0]['filename']
            image = cv2.imread(image_path)
            # 读取提取的数字
            extracted_file_path = 'extracted_numbers.txt'  # 修改为你的文件路径
            extracted_numbers = load_numbers_from_txt(extracted_file_path)
            lidar2img = curr_batch_dict['img_metas'][0]['lidar2img']
            # 将提取的数字转换为整数
            extracted_numbers = [int(num) for num in extracted_numbers if num.isdigit()]

            # 检查 sample_idxs 中的任意值是否在 extracted_numbers 中
            matching_idxs = [idx for idx in sample_idxs if idx in extracted_numbers]

            if iter > 4000 and matching_idxs is not None :
            # a =2
            # if a>1 :
                for batch_idx, (curr_points, curr_sample_idx) in \
                    ...
                    if 'gt_bboxes' in curr_batch_dict:
                        gt_bboxes = apply_3d_transformation_bboxes(
                            ...
                            img_metas[batch_idx],
                            reverse=True)
                    if tea_bboxes is None or tea_bboxes[batch_idx] is None:
                        continue
                    else:
                    ...
                    canvas.draw_3d_box(image, curr_tea_bboxes[:, :7],lidar2img,sample_idxs)
                    ### Put into dict
        ...
        save_dir = "/home/lab/SS/environment/vis_pvrcnn/"
        if vis_dict :
            for sample_idx, canvas_image in vis_dict.items():
                save_path = os.path.join(save_dir, f"sample_{sample_idx}.png")
                plt.imsave(save_path, canvas_image)
